...
const ChatBubble = ({ content, time, subject, img }: ChatInputProps) => {
  const justifyStyle =
    subject === "me"
      ? "flex justify-end items-center mt-3"
      : "flex justify-start items-center mt-3  relative";
  const flexStyle =
    subject === "me"
      ? "flex flex-row items-end px-2   "
      : "flex flex-row-reverse items-end pl-11";
    ...
    <div className={justifyStyle}>
      ...
      <div className={flexStyle}>
        ...
        {img && (
          <img
            src={img}
            className="w-9 rounded-full border mr-2 absolute left-0 top-0"
          />
        )}
      </div>
    </div>
