...
const ChatBubble = ({ content, time, subject, img, read, profile_img }: ChatInputProps) => {
  ...
  const showOpponentImage = subject !== `${userData?.nickname}` && profile_img;
  ...
  const justifyStyle =
    subject === `${userData?.nickname}` ? 'flex justify-end items-center mt-3' : 'flex justify-start items-center mt-3  relative';
  const flexStyle = subject === `${userData?.nickname}` ? 'flex flex-row items-end px-2   ' : 'flex flex-row-reverse items-end ';

  const inputColor =
    subject === `${userData?.nickname}`
      ...
      <div className={flexStyle}>
        ...
        {img ? (
          <div className="flex flex-col gap-y-2">
            {/* 이미지가 있을 때 */}
            {content && <div className={inputColor}>{content}</div>}
