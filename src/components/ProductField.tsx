...
interface ProductFieldProps {
  label: string;
  placeholder?: string;
  type: 'text' | 'select' | 'textarea' | 'radio' | 'date' | 'number';
  maxLength?: number;
  suffix?: string;
...
const ProductField = forwardRef<HTMLTextAreaElement | HTMLInputElement | HTMLSelectElement, ProductFieldProps>(
  ({ label, placeholder, type, maxLength, suffix, options, height, textAreaDetail }, ref) => {
    const renderField = () => {
      if (type === 'select') {
        return (
          <div className="flex items-center justify-center w-full">
            ...
            <select
              ...
              className="shadow appearance-none border rounded w-3/4 md:w-8/12 text-mainBlack leading-tight focus:outline-none focus:shadow-outline focus:border-mainWhite focus:bg-mainWhite"
            >
              {options &&
